...
Use the `text` attribute (not `key`) to specify which translation to use:
...
**Important:** Always use the `text` attribute for translations with `ds-text`. The `key` attribute is not supported. If a translation is not found, the component will fall back to displaying the `text` attribute value itself.
...
// Use with dot notation - always use the `text` attribute
<ds-text text="nav.home"></ds-text>;
```
...
<html lang="en">
  ...
  <body>
    <div id="app">
      <ds-text variant="heading" text="Title"></ds-text>
      <ds-text variant="body" text="Description"></ds-text>
