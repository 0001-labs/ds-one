...
Use the `text` attribute (not `key`) to specify the translation key:
...
**Note:** Always use the `text` attribute for translations with `ds-text`. The `key` attribute is not supported.
