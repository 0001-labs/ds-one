...
import { currentLanguage, translate } from "../DS1/0-face/i18n";

/**
 * Download CV button that resolves language-specific URLs
 *
 * @element portfolio-downloadcv
 ...
 */
export class PortfolioDownloadCV extends LitElement {
  static properties = {
  ...
  private async _download() {
    if (!this._activeUrl) {
      console.warn("[portfolio-downloadcv] No URL available");
      return;
    }

    console.log(
      "[portfolio-downloadcv] Attempting download from:",
      this._activeUrl
    );
    ...
    try {
      ...
      console.log(
        "[portfolio-downloadcv] Fetch response:",
        res.status,
        res.statusText
      );
      ...
      console.log("[portfolio-downloadcv] Download triggered successfully");
    } catch (err) {
      console.error(
        "[portfolio-downloadcv] Fetch failed, using fallback:",
        err
      );
...
customElements.define("portfolio-downloadcv", PortfolioDownloadCV);

declare global {
  interface HTMLElementTagNameMap {
    "portfolio-downloadcv": PortfolioDownloadCV;
  }
}
